...
- **TAB 官方直连**：TAB、编辑历史、代码指标和文件同步接口保持连接 Cursor 官方服务。
...
TAB 补全、编辑历史、AI 代码修改指标、Git 分支名和文件同步接口原样连接 Cursor 官方服务，不提供公共中转或自定义 TAB 服务配置。

Commit Message 默认关闭，不会转发 Cursor 的原始请求。只有在系统设置中明确选择已配置模型后，才会通过该模型本地生成提交信息。
